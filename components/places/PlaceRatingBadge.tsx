...
export function PlaceRatingBadge({ rating, userRatingsTotal, className, compact = false }: PlaceRatingBadgeProps) {
  ...
  return (
    <span
      ...
      title={hasReviews ? `${formatRating(rating)} en Google Places con ${userRatingsTotal} reseñas` : `${formatRating(rating)} en Google Places`}
    >
      ...
      {hasReviews ? <span className="font-semibold text-zinc-500">({compact ? formatRatingsTotal(userRatingsTotal) : `${formatRatingsTotal(userRatingsTotal)} reseñas`})</span> : null}
    </span>
  );
}
