...
describe("group activity lib", () => {
  ...
  it("summarizes groups with recent activity", async () => {
    createSupabaseServerClientMock.mockResolvedValue({
      from: vi.fn((table: string) => {
        if (table === "group_members") {
          return {
    ...
    const { getGroupsWithRecentActivityForUser } = await import("@/lib/groupActivity");
    const result = await getGroupsWithRecentActivityForUser("me", 5);
