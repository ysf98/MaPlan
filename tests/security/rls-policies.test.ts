...
describe("RLS policies baseline", () => {
  ...
  it("includes group plans tables, unique safeguards and member RLS policies", () => {
    ...
    expect(sql).toContain("drop function if exists public.get_public_group_plan(uuid)");
    ...
    const publicPlanFunction = sql.slice(
      sql.indexOf("create or replace function public.get_public_group_plan"),
      sql.indexOf("revoke execute on function public.get_public_group_plan")
    );
    expect(publicPlanFunction).not.toContain("group_plan_votes");
    expect(publicPlanFunction).not.toContain("group_members");
